...
const variableName = computed(() => `{{ ${data.variableName} }}`);
</script>
...
.title {
  color: var(--text-input-node-color);
  padding: 0.3rem;
  display: flex;
  flex-direction: row;
  align-items: center;
...
.title > svg {
  height: 0.6rem;
  fill: var(--text-input-node-color);
  padding: 2px;
  border-radius: 2px;
}
...
.edit-button:hover {
  background: var(--text-input-node-color);
  fill: white;
}
...
.socket {
  background: var(--text-input-node-color) !important;
  border: none;
  top: 12px;
}
...
.socket:hover {
  background: rgb(from var(--text-input-node-color) r g b / 80%) !important;
  -webkit-box-shadow: 0px 0px 3px 1px var(--text-input-node-color);
  -moz-box-shadow: 0px 0px 3px 1px var(--text-input-node-color);
  box-shadow: 0px 0px 3px 1px var(--text-input-node-color);
  border: none;
}
